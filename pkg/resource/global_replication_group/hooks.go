...
func (rm *resourceManager) modifyGlobalReplicationGroup(
	...
	cpgnChanged := desired.ko.Spec.CacheParameterGroupName != nil &&
		!strPtrEqual(desired.ko.Spec.CacheParameterGroupName, latest.ko.Spec.CacheParameterGroupName)

	// CacheParameterGroupName is only valid as part of a major engine upgrade
	// (it is sent together with Engine + EngineVersion in the engineGroupChanged
	// branch). A CPGN change on its own -- whether it is the only pending change
	// or it accompanies an unrelated change like Description -- is a permanent
	// user misconfiguration AWS rejects, so surface it as terminal (recovers
	// when the user corrects the spec) rather than swallowing it into another
	// branch or requeuing forever.
	if cpgnChanged && !engineGroupChanged {
	...
	// the rest (see below). CacheParameterGroupName is not counted separately:
	// a valid CPGN change only ever travels with engineGroupChanged (already
	// counted), and a standalone CPGN change is handled as terminal above.
	pending := 0
	...
	// so its "observed" value is tracked in an annotation. Record it only when
	// it was actually sent to AWS (as part of the engine-upgrade branch above),
	// so a value that was never sent is never falsely reported as applied.
	out, err := rm.setGlobalReplicationGroupOutput(desired, resp.GlobalReplicationGroup)
	if err != nil {
		return nil, err
	}
	if input.CacheParameterGroupName != nil {
		rm.setLastRequestedCPGN(desired, out.ko)
	}
	if pending > 1 {
...
// setLastRequestedCPGN records the CacheParameterGroupName value that was just
// sent to AWS in an annotation. The Describe API does not return this field and
// it is not derivable, so this annotation is the only way a subsequent reconcile
// can tell what value is actually in effect. It is written only after a Modify
// that carried CacheParameterGroupName, so a value that was never sent is never
// recorded as applied.
func (rm *resourceManager) setLastRequestedCPGN(
	r *resource,
	ko *svcapitypes.GlobalReplicationGroup,
) {
